...
use kali\core\lib\cls_securimage;
use kali\core\lib\cls_google_auth;
use kali\core\lib\cls_auth;
use kali\core\lib\cls_menu;
use kali\core\lib\cls_redis;
use kali\core\lib\cls_redis_lock;
use kali\core\lib\cls_snowflake;
...
class ctl_index
{
    ...
    {
        self::$config = config::instance('app_config')->get('common');
        $security = config::instance('app_config')->get('security');
        self::$config = array_merge(self::$config, $security['validate']);
    }
    ...
    public function index()
    {
        ...
        // websocket
        $conf = config::instance('app_config')->get('websocket');
        $url_websocket = '';
        if ( $conf['enable'] )
        {
            $key        = util::random('web');
    ...
    public function login_otp()
    {
        ...
        if ( req::method() == 'POST' )
        {
            ...
            if ( $ret )
            {
                $user = cls_auth::instance($uid)->get_user();
    ...
    public function reset_pwd()
    {
        ...
        if ( req::method() == 'POST' )
        {
            ...
            try
            {
                ...
                // 使用了之前相同的密码
                if ( cls_auth::password_hash($password) == $user['password'] )
                {
                ...
                cls_auth::instance($uid)->save_user($data);
