...
class ctl_admin
{
        ...
        if (!empty(req::$posts))
        {
            ...
            db::insert('#PB#_admin')
                ->set(array(
                    'uid'            => $uid,
                    'username'       => $username,
                    'password'       => cls_auth::password_hash($password),
    ...
    public function edit()
    {
        ...
        if (!empty(req::$posts))
        {
            ...
            if( $password != '' )
            {
                $data['password'] = cls_auth::password_hash($password);
            }
            db::update('#PB#_admin')
                ->set($data)
                ->where('uid', $id)
            ...
            cls_auth::instance($id)->del_cache();
    ...
    public function active()
    {
        ...
        if ( $is_active )
        {
            foreach ($ids as $id)
            {
                cls_auth::instance($id)->del_login_error24();
            }
    ...
    public function purview()
    {
        ...
        if (!empty(req::$posts))
        {
            $group_purviews = cls_auth::instance($id)->get_group_purviews();
        ...
        else
        {
            $info = db::select('username, realname, groups')
                ...
                ->execute();
            // 具体用户的权限
            $info['purviews'] = cls_auth::instance($id)->get_purviews();
            $groups = explode(',', $info['groups']);
            $info['groupname'] = cls_auth::instance($id)->get_groupname($groups);
