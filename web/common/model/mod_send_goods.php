...
class mod_send_goods extends mod_base
{
    ...
    public static function save($data, $where = [])
    {
        ...
        if(empty($where) && empty($data[self::$_pk] && $result))
        {
            ...
            $date = date('Y-m-d H:i:s', $member_info['go_off']);
            $content = "尊敬的{$member_info['username']} 先生/女生，您的货物已于{$date}装车并出发，车牌号码是：{$member_info['car_num']}";
    ...
    public static function send_sms($mobile,$content)
    {
        ...
        log::info('发货短信通知结果: ' . var_export($res, 1), __METHOD__);
    }
}