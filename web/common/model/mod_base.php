...
class mod_base
{
    ...
    public static function save($data, $where = [])
    {
        ...
        if(empty($where) && empty($data[static::$_pk]) )
        {
            ...
            {
                $query = db::insert(static::$_table)->columns(array_keys(current($data)))->values($data);
            }
            else
            {
        ...
        else
        {
            ...
            if(is_array(current($data)))
            {
                $query = db::update(static::$_table)->columns(array_keys(current($data)))->values($data);
            }
            else
            {
    ...
    public static function del($where)
    {
        ...
        return db::delete(static::$_table)->where($where)->execute();
